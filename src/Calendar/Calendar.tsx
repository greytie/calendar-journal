...
const Month = ({ date: initalDate }: {date: Date}) => {
    const [currentDate, setDateStore] = React.useState(initalDate)

    const changeMonth = (monthChange: number)  => {
 ...
 * @returns 
 */
const DayCell = ( { date, currentMonthIndex }: { date: Date, currentMonthIndex: number } ) => {
...
const getDateColor = (isCurrentMonth: boolean, isDateSunday: boolean) => {
    if (isCurrentMonth) {
        if (isDateSunday) {
